...
// assignment operator
uint64_t numbers::operator = ( const uint64_t num ) {
    setnum(num);
    return getnum();
}

const char * numbers::words() {
    return words(_num);
}

// convert to words
const char * numbers::words( const uint64_t num ) {
    if (num > _MAXNUM) {
        return errnum;
    }
    initbuf();
...
void numbers::initbuf() {
    clearbuf();
    _buf = (char *) malloc(_NUMWORD_MAXSTR);
    *_buf = 0;
    hyphen_flag = false;
}

// append space (or hyphen)
void numbers::appendspace() {
    if (_buflen) {
...
void numbers::appendbuf(const char * s) {
    if(!s) return;
    size_t slen = strnlen(s, _NUMWORD_MAXSTR);
    if (slen < 1) {
        return;
    }
    if ((slen + _buflen + 1) >= _NUMWORD_MAXSTR) {
        return;
    }
